...
int main() {
    ...
    if (pid == 0) { // Child process
	  ...
	  printf("Child: closing stdin (fd 0)\n");
        ...
        char buffer[10];
        printf("Enter elements for read buffer\n");
        ...
        if (result < 0) {
            ...
            printf("Parent: stdin is still open fd:%d\n",fd);
        }

        int fresult= close(fd); // Close the file descriptor
        if(fresult ==0){
           printf("Parent : fd is closed=%d\n",fd);
	}else{
	   printf("parent : fd is not closed=%d\n",fd);
	}
    }
